...
import type { Client, Devis, LigneFacture } from '@/types'
...
export function FactureFormPage() {
  ...
  useEffect(() => {
    if (clientId) {
      supabase.from('devis').select('*').eq('client_id', clientId).eq('statut', 'accepte')
        .then(({ data }) => setDevisList(data as Devis[] ?? []))
    } else {
      setDevisList([])
    }
  }, [clientId])
  ...
  useEffect(() => {
    ...
    const loadEdit = async () => {
      ...
      if (f.data) {
        setClientId(f.data.client_id ?? '')
        setDevisId(f.data.devis_id ?? '')
        setDateEmission(f.data.date_emission ?? '')
        setDateEcheance(f.data.date_echeance ?? '')
        setConditions(f.data.conditions_paiement ?? '')
        setRemise(f.data.remise_pct ?? 0)
        setNotes(f.data.notes ?? '')
        setMentions(f.data.mentions_legales ?? '')
      }
      if (l.data) {
        setLignes(l.data.map((lf: LigneFacture) => ({
          id: uid(), description: lf.description, quantite: lf.quantite,
          prix_unitaire_ht: lf.prix_unitaire_ht, taux_tva: lf.taux_tva,
        })))
      }
    }
    loadEdit()
  ...
  const handleSave = async () => {
    ...
    try {
      let factureId = id
      if (!isEdit) {
        const { data: numData } = await supabase.rpc('next_numero_facture')
      ...
      const { error: lignesErr } = await supabase.from('lignes_facture').insert(
        lignes.map((l, i) => ({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-heading text-xl font-bold text-text-primary">{isEdit ? 'Modifier la facture' : 'Nouvelle facture'}</h1>
      </div>
      ...
      <div className="bg-bg-surface border border-border-color rounded-2xl p-6">
        {step === 0 && (
          <div className="space-y-4">
            <h2 className="font-heading font-semibold text-lg text-text-primary mb-4">Client</h2>
            <div>
            ...
            {clientId && devisList.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-text-primary mb-1">Devis associé (optionnel)</label>
                <select value={devisId} onChange={e => setDevisId(e.target.value)} className={cls}>
                  <option value="">Aucun devis</option>
                  {devisList.map(d => <option key={d.id} value={d.id}>{d.numero}</option>)}
                </select>
              </div>
            )}
          </div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-text-primary mb-1">Date d'émission *</label>
                <input type="date" value={dateEmission} onChange={e => setDateEmission(e.target.value)} className={cls} />
              </div>
              <div>
                <label className="block text-sm font-medium text-text-primary mb-1">Date d'échéance</label>
                <input type="date" value={dateEcheance} onChange={e => setDateEcheance(e.target.value)} className={cls} />
              </div>
            </div>
            <div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-text-primary">{dateEcheance || '—'}</p>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-bg-surface">
                    <th className="px-4 py-2 text-left text-xs text-text-muted">Description</th>
                    <th className="px-4 py-2 text-right text-xs text-text-muted">Qté</th>
                    <th className="px-4 py-2 text-right text-xs text-text-muted">PU HT</th>
                    <th className="px-4 py-2 text-right text-xs text-text-muted">Total HT</th>
                  </tr>
                </thead>
                <tbody>
                  {lignes.map((l, i) => (
      ...
      <div className="flex items-center justify-between">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <Button onClick={handleSave} loading={saving}>
            <Check className="w-4 h-4" /> {isEdit ? 'Mettre à jour' : 'Créer la facture'}
          </Button>
        )}
      </div>
    </div>
